...
public class DuelServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (player==null || !player.hasBattle() || !player.getBattle().isStartGame()
                || player.getBattle().isExit(player.getIdInBattle())) {
            resp.sendRedirect("/");
            return;
        }
    ...
    private HashMap<String, String> getParams (Battle battle, int playerId) {
        ...
        params.put("his_life", Long.toString(battle.getEnemyLife(playerId)));
        ...
        if (battle.isExit(playerId))
            params.put("text_button", "Выйти");
        else
            params.put("text_button", "Атаковать");
